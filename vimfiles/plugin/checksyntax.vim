...
if !exists('g:checksyntax_use_make_cmd')
    let g:checksyntax_use_make_cmd = 0
endif
...
if !exists('g:checksyntax_cmd_javascript')
    if exists('g:jslint_command_options')
        let g:checksyntax_cmd_javascript = 'jsl ' . g:jslint_command_options
    else
...
" endif
"if !exists('g:checksyntax_okrx_javascript')
"    let g:checksyntax_failrx_javascript = ''
"endif
    ...
    try
        if g:checksyntax_use_make_cmd
            ...
            if &makeprg[0:0] == '*'
                ...
                exec 'make ' . iconv(shellescape(expand('%:p')), &enc, 'gbk')
            endif
            ...
            let cmd = &makeprg . ' ' . shellescape(expand('%:p'))
            if has("gui_win32")
                let cmd = iconv(cmd, &enc, 'gbk')
            endif
            let errors = system(cmd)
            if has("gui_win32")
                let errors = iconv(errors, 'gbk', &enc)
            endif
...
function! CheckSyntax(manually, ...)
    ...
    try
        ...
        for i in qflist
            ec i
            if i.lnum != 0
                if has("gui_win32") && g:checksyntax_use_make_cmd
                    let i.text = iconv(i.text, "gbk", &enc)
                endif
                call add(new_qflist, i)
            endif
        endfor
